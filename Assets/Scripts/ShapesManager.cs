...
{
	public Button objPlayButton;
	public Button objReStartButton;
	...
	private int score;
	public readonly Vector2 BottomRight = new Vector2 (-1.8f, -2.7f);
...
//		}
	...
	void Update ()
	{
		...
		if (!bGameOver && false == bPause) {
			
			fTimer -= Time.deltaTime;
			textTimer.text = ":" + fTimer.ToString ("0");
			print (fTimer);
			if (fTimer <= 0) {
	...
	public void ShowPlayButton ()
	{
		...
		objReStartButton.gameObject.SetActive (true);
	}

	public void HidePlayButton ()
	{
		if (WonText.activeSelf) {
			HideWonText ();
			//	InitializeCandyAndSpawnPositions ();
		}
		...
		if (bPause == false) {
			...
			objReStartButton.gameObject.SetActive (false);


		} else {

			bPause = false;
			objPlayButton.gameObject.SetActive (false);
			objReStartButton.gameObject.SetActive (false);
		}
	...
	{
		bPause = true;
		objPlayButton.gameObject.SetActive (true);
		objReStartButton.gameObject.SetActive (true);
	}
